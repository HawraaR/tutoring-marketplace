...
const hoursBySubject = [
  { code: "MATH 201", title: "Calculus II", hours: 6, tone: "burgundy" as const, sessions: 5 },
  { code: "CHEM 240", title: "Organic Chemistry", hours: 3.5, tone: "olive" as const, sessions: 3 },
  { code: "SPAN 310", title: "Conversation", hours: 2, tone: "slate" as const, sessions: 2 },
];
...
const tutors = [
  { name: "Layla Hassan", credentials: "MSc", dept: "Mathematics", rate: "$32/hr", last: "14 Aug", sessions: 5 },
  { name: "Omar Reid", credentials: "PhD cand.", dept: "Chemistry", rate: "$38/hr", last: "9 Aug", sessions: 3 },
  { name: "Sofia Alvarez", credentials: "MA", dept: "Modern Languages", rate: "$28/hr", last: "2 Aug", sessions: 2 },
  { name: "Kenji Ito", credentials: "MEng", dept: "Computer Science", rate: "$35/hr", last: "28 Jul", sessions: 1 },
];
  ...
  return (
    <div className="mx-auto grid max-w-7xl gap-2 lg:grid-cols-[minmax(0,1fr)_300px]">
      <div className="flex min-w-0 flex-col gap-2">
        <section className="grid gap-2 lg:grid-cols-2">
          <article className="rounded-sm border border-border-subtle bg-surface-card p-4">
            <div className="mb-2 flex items-baseline justify-between gap-2">
              ...
              <p className="text-[11px] text-muted">Autumn 2026 · {hoursTotal.toFixed(1)} hrs</p>
            </div>
            <table className="w-full text-xs">
              ...
              <tbody>
                {hoursBySubject.map((row) => (
                  <tr key={row.code} className="border-b border-border-subtle last:border-0">
                    ...
                    <td className="py-2">
                      <div className="h-1.5 bg-border-subtle">
                        <div
                          ...
                          style={{ width: `${(row.hours / hoursTotal) * 100}%` }}
                        />
                      </div>
                    </td>
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              ...
              <tbody>
                {sessions.map((session) => (
                  <tr key={session.id} className="border-b border-border-subtle last:border-0 align-top">
                    ...
                    <td className="py-3 pr-3 text-xs text-muted">{session.mode}</td>
                    <td className="py-3 whitespace-nowrap text-right">
                      ...
                      <button type="button" className="text-sm text-muted hover:text-ink hover:underline">
                        Reschedule
                      </button>
                    </td>
                  </tr>
        ...
        <section className="rounded-sm border border-border-subtle bg-surface-card p-4">
          <h2 className="font-serif mb-3 text-lg font-semibold text-ink">Recent tutors</h2>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[560px] text-left text-sm">
              ...
              <tbody>
                {tutors.map((tutor) => (
                  <tr key={tutor.name} className="border-b border-border-subtle last:border-0">
                    <td className="py-2.5 pr-3">
                      <span className="font-medium text-ink">{tutor.name}</span>
                      <span className="ml-1.5 text-xs text-muted">{tutor.credentials}</span>
                    </td>
                    <td className="py-2.5 pr-3 text-muted">{tutor.dept}</td>
                    <td className="py-2.5 pr-3 tabular-nums text-ink">{tutor.rate}</td>
                    <td className="py-2.5 pr-3 tabular-nums text-muted">{tutor.sessions}</td>
      ...
      <aside className="flex flex-col gap-2">
        <section className="rounded-sm border border-border-subtle bg-surface-card p-4">
          <h2 className="font-serif text-base font-semibold text-ink">Shortcuts</h2>
          <ul className="mt-2 divide-y divide-border-subtle text-sm">
            <li>
              <button type="button" className="w-full py-2 text-left text-brand-primary hover:underline">
        ...
        <section className="rounded-sm bg-brand-primary p-4 text-white">
          ...
          <p className="mt-2 text-sm leading-relaxed text-white/70">
            Applicants must be currently enrolled and have completed the listed course with a mark of B or higher. Hourly rates and payouts are set in your tutor profile.
          </p>
          <button
            type="button"
