...
export function MessagesIO() {
  ...
  // ── Refs for race-condition & socket management ───────────────────────────
  ...
  // useSocket hook ___________________________________________
  const { joinConversation, leaveConversation } = useSocket({
    ...
    onNewMessage: (message) => {
      ...
      if (message.conversationId === activeId) {
        ...
        markConversationRead(message.conversationId).catch(console.error);
      }

      // Update conversation list preview & unread counts globally
      setConversations((prevItems) =>
        prevItems.map((item) => {
          if (item.id === message.conversationId) {
            const isCurrentActive = item.id === activeId;
            return {
              ...item,
              updatedAt: message.createdAt,
              unreadCount:
                !isCurrentActive && message.senderId !== user?.id
                  ? item.unreadCount + 1
                  : item.unreadCount,
              messages: [
                {
  ...
  // 3. Handle joining/leaving specific conversation rooms when selection changes
  useEffect(() => {
    if (selectedId) {
  ...
  }, [requestedConversationId, navigate]);

  // ── Initialize Socket.io Connection ───────────────────────────────────────
  // useEffect(() => {
  //   // Connect to your backend socket server (adjust URL/options as needed)
  ...
  //     // Re-join current active room if already selected
  //     if (selectedIdRef.current) {
  //       socket.emit("join_conversation", selectedIdRef.current);
  //     }
  //   });
  ...
  // // Handle joining/leaving specific conversation rooms when selection changes
  // useEffect(() => {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      // Emit the message over socket server if your backend expects client-emitted messages,
      // or rely on API response handling if your backend broadcasts from the sendMessage controller.
      // socketRef.current?.emit("send_message", { conversationId: selectedId, message });
...
// export const MessagesIO: React.FC<ChatPageProps> = ({ token, currentUserId }) => {
//   const [activeConversationId, setActiveConversationId] = useState<string | null>(null);
//   const [latestConversation, setLatestConversation] = useState<Conversation | null>(null);

//   // Top-level socket listener for brand-new conversations coming in real-time
//   useSocket({
//     token,
...
//   return (
//     <div className="flex h-[calc(100vh-4rem)] w-full overflow-hidden border border-gray-200 rounded-xl">
//       <ConversationsList
//         token={token}
