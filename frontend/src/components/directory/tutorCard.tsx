...
function AvailabilityHint({ tutor }: { tutor: TutorListItem }) {
  ...
  if (!next)
    return <span className="text-xs text-muted">No open slots this week</span>;
  const day = new Date(next.startTime).toLocaleDateString("en-US", {
    weekday: "short",
  });
  return (
  ...
  return (
    <div className="flex items-center gap-2">
      <a
        ...
        className="rounded-lg px-3 py-2 text-sm font-semibold text-brand-primary hover:bg-brand-primary/5"
      >
        Details
      </a>
      <button
...
function NameRow({ tutor }: { tutor: TutorListItem }) {
  return (
    <span className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5">
      <h3 className="min-w-0 font-serif text-lg font-semibold text-ink">
        {displayName(tutor)}
      </h3>
