...
import { DAY_OPTIONS, PRICE_BOUNDS, type TutorFilters } from '../../lib/directory/tutorFilters';
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">{children}</p>;
}
...
export function FilterPanel({ filters, onChange, onClear, subjects }: Props) {
  ...
  const toggleDay = (d: number) =>
    onChange({ ...filters, days: filters.days.includes(d) ? filters.days.filter((x) => x !== d) : [...filters.days, d] });
    ...
    <div className="flex flex-wrap items-start gap-6 rounded-xl border border-border-subtle bg-surface-card p-4 shadow-warm-sm">
      ...
      <div className="min-w-[15rem] sm:w-64">
        ...
        <div className="relative h-5">
          ...
          />
          <input
            type="range" min={PRICE_BOUNDS.min} max={PRICE_BOUNDS.max} step={1} value={filters.priceMin}
            onChange={(e) => onChange({ ...filters, priceMin: Math.min(Number(e.target.value), filters.priceMax) })}
            className="range-input" style={{ zIndex: filters.priceMin > PRICE_BOUNDS.max - span / 10 ? 30 : 20 }}
            aria-label="Minimum hourly rate"
          />
          <input
            type="range" min={PRICE_BOUNDS.min} max={PRICE_BOUNDS.max} step={1} value={filters.priceMax}
        ...
        <div className="mt-2 flex items-center gap-2">
          <span className="flex-1 rounded-md border border-border-subtle bg-surface-bg px-2 py-1 text-center text-xs font-semibold text-ink">${filters.priceMin}</span>
          <span className="text-xs text-muted">to</span>
          <span className="flex-1 rounded-md border border-border-subtle bg-surface-bg px-2 py-1 text-center text-xs font-semibold text-ink">${filters.priceMax}</span>
        </div>
      </div>
      ...
      <div className="min-w-[16rem] flex-1">
        ...
        <p className="mt-1.5 text-[11px] text-muted">Shows tutors with an open slot on the selected days.</p>
      </div>

      {/* ── Clear button ─────────────────────────────── */}
      <div className="flex w-full items-center justify-end sm:w-auto sm:pt-6">
        <button onClick={onClear} className="text-sm font-semibold text-slate-blue hover:underline">Clear All</button>
      </div>
    </div>
  );
}
...
//         <div className="relative h-5">
//           <div className="absolute top-1/2 h-1.5 w-full -translate-y-1/2 rounded-full bg-border-subtle" />
//           <div className="absolute top-1/2 h-1.5 -translate-y-1/2 rounded-full bg-slate-blue" style={{ left: `${minPct}%`, right: `${100 - maxPct}%` }} />
...
//           <label className="flex-1 rounded-lg border border-border-subtle bg-surface-card px-3 py-2 text-center text-sm font-semibold text-ink">
//             ${filters.priceMin}
//           </label>
...
//           })}
//         </div>
//         <p className="text-xs text-muted">Shows tutors with at least one open slot on the selected days.</p>
//       </div>
//     </div>
//   );
// }